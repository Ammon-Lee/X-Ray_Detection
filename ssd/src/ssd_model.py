...
class Backbone(nn.Module):
    def __init__(self, pretrain_path=None):
        ...
        # Modify the stride space of conv4_block1 from 2->1
...
class SSD300(nn.Module):
    ...
    def _build_additional_features(self, input_size):
        """
        Backbone (Resnet50) added a series of extra convolution layer, get the corresponding series of feature extractor
        :param input_size:
        :return:
        """
        additional_blocks = []
    ...
    def forward(self, image, targets=None):
        ...
        # The prediction regression parameters are superimposed on the default box to get the final prediction box,
        # and the non-maximum suppression is performed to eliminate the overlap box
...
class Loss(nn.Module):
    ...
    def _location_vec(self, loc):
        ...
        Calculate the regression parameters of ground truth relative to anchors
        :param loc: The corresponding GTBox Nx4x8732 matched by the anchor
        :return:
        """
    ...
    def forward(self, ploc, plabel, gloc, glabel):
        ...
        """
        # Obtain positive sample of Mask Tensor: [N, 8732]
        mask = glabel > 0
        # mask1 = torch.nonzero(glabel)
        # Calculate the number of positive samples for each image in a batch. Tensor: [N]
        pos_num = mask.sum(dim=1)

        # Calculate the location regression parameter of gt. Tensor: [N, 4, 8732]
        vec_gd = self._location_vec(gloc)

        # sum on four coordinates, and mask
        # Calculate location loss (positive sample only)
        ...
        # Getting a negative sample
        con_neg = con.clone()
        con_neg[mask] = 0.0
        # Sort in descending order by confidence_loss con_idx(Tensor: [N, 8732])
        _, con_idx = con_neg.sort(dim=1, descending=True)
        _, con_rank = con_idx.sort(dim=1)  # 这个步骤比较巧妙

        # number of negative three times positive
        # The number of negative samples used for loss calculation is 3 times that of positive samples (in the original Hard Negative Mining),
        # but the total sample size should not exceed 8732
        neg_num = torch.clamp(3 * pos_num, max=mask.size(1)).unsqueeze(-1)
        neg_mask = con_rank < neg_num  # Tensor [N, 8732]

        # Confidence Final Loss uses the selected positive sample loss+ the selected negative sample loss
        con_loss = (con * (mask.float() + neg_mask.float())).sum(dim=1)  # Tensor [N]

        # avoid no object detected
        # Avoid situations where there is no GTBox in the image
        total_loss = loc_loss + con_loss
        # eg. [15, 3, 5, 0] -> [1.0, 1.0, 1.0, 0.0]
        num_mask = (pos_num > 0).float()  # Statistics whether there is a positive sample in each image in a batch
        pos_num = pos_num.float().clamp(min=1e-6)  # keep the denominator from being zero
        ret = (total_loss * num_mask / pos_num).mean(dim=0)  # Only the image loss with positive samples is calculated
        return ret
